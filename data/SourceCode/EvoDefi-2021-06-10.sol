...
contract MasterChef is Ownable, ERC721Holder {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 genReward = blockAmount
            .mul(genPerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        gen.mint(devaddr, genReward.div(10));
        gen.mint(address(this), genReward);
        pool.accGenPerPower = pool.accGenPerPower.add(
            genReward.mul(1e12).div(pool.totalPower)
        );
        pool.lastRewardBlock = block.number;
    }
        ...
        for (uint i = 0; i < _nftIds.length; i++) {
            ...
            if (nft.ownerOf(nftId) == msg.sender) {
                if (user.lastPoweredBlock >= block.number) {
                    user.lastPoweredBlock += proxy.getNFTPowerBonusBlocks(
                        nftId
                    );
                } else {
                    user.lastPoweredBlock =
                        block.number +
                        proxy.getNFTPowerBonusBlocks(nftId);
                }
    ...
    function _pendingGen(
        ...
        if (
            ...
            uint256 genReward = blockAmount
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accGenPerPower = accGenPerPower.add(
                genReward.mul(1e12).div(pool.totalPower)
            );
        }

        if (
            (user.lastClaimedBlock < user.lastPoweredBlock) &&
            ...
            powerBonus +=
                (proxy.getNFTPowerBonus() *
                    (user.lastPoweredBlock - user.lastClaimedBlock)) /
                (block.number - user.lastClaimedBlock);
            powerBonus += proxy.bonus(msg.sender);

            uint truePower = user.amount.mul(powerBonus).div(100);
            return
                (user.power.mul(accGenPerPower).div(1e12).sub(user.rewardDebt) *
                    truePower) / user.power;
        }
